...
def run_level(screen, clock, level: str) -> str:
    ...
    while running:
        ...
        # Fixed Enemy Collision (Checks in true world space coordinates)
        for enemy in enemy_data:
        ...
        if is_dead:
            ...
            if choice == "restart":
                return run_level(screen, clock, level)  # Loops back cleanly to restart the level
            elif choice == "leave":
                return "levels"
        ...
        for collider in end:
            if player.colliderect(collider):
                ...
                if choice == "restart":
                    return run_level(screen, clock, level)  # Replays current level cleanly
                elif choice == "leave":
        ...
        text = nadpisy.render(f"LEVEL {level_num}", True, white)
